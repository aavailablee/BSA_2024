...
class Momentum_batch_learnable(nn.Module):
    def __init__(self, configs, vector_len):
        ...
        momentum_params = torch.asarray(configs.momentum_params)
        momentum_params_learnable = torch.log(torch.div(momentum_params,(1-momentum_params)))
        self.momentum_params_learnable = nn.Parameter(momentum_params_learnable.unsqueeze(1).unsqueeze(2))

        self.exp_mtx_1 = torch.zeros(1, self.batch + 1, self.batch)
        self.exp_mtx_2 = torch.zeros(1, self.batch + 1, self.batch)
    ...
    def gen_mul_tensor(self, momentum_params_learnable):
    ...
    def forward(self, vector): # B, vector_len | torch.Size([64, 96])
        ...
        if batch == self.batch:
            out = torch.matmul(lhs, self.mul_tensor) # out: 3,F,B
        else:
            out = torch.matmul(lhs, self.mul_tensor[:, :batch+1, :batch])  # out: 3,F,b
