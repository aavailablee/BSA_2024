...
from layers.SelfAttention_Family import TwoStageAttentionLayer, TwoStageAttentionLayer_custom
...
class scale_block(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        return x, None #TODO 여기 또 attns return 값 없다
    
class scale_block_custom(nn.Module):
    def __init__(self, configs, win_size, d_model, n_heads, d_ff, depth, dropout, seg_num=10, factor=10):
        super(scale_block_custom, self).__init__()
        ...
        for i in range(depth):
            self.encode_layers.append(TwoStageAttentionLayer_custom(configs, seg_num, factor, d_model, n_heads, \
                                                             d_ff, dropout))
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return encode_x, None #TODO 여기서 attns 에 아무것도 리턴이 없어서 문제인거임 여길 해결해야 하니까 원래 코드랑 비교해보자 
